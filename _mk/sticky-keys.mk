...
uninstall-sticky-keys: ## SHIFTキー固定モード対策をアンインストール
	...
	@{ \
	  ...
	  if echo "$$EXISTING" | grep -Fq "$$TARGET"; then \
	    PRUNED=$$(python3 -c 'import ast,sys; lst=ast.literal_eval(sys.argv[1]); target=sys.argv[2]; lst=[x for x in lst if x!=target]; print("[" + ",".join(f"'\''{x}'\''" for x in lst) + "]")' "$$EXISTING" "$$TARGET"); \
